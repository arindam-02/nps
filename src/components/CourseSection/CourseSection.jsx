import React from "react";

const CourseSection = () => {
  return (
    <>
      <section className="ftco-section">
        <div className="container">
          <div className="row justify-content-center mb-5 pb-2">
            <div className="col-md-8 text-center heading-section">
              {/* <div className="col-md-8 text-center heading-section ftco-animate"> */}
              <h2 className="mb-4">
                <span>Our</span> Courses
              </h2>
              <p>
                Separated they live in. A small river named Duden flows by their
                place and supplies it with the necessary regelialia. It is a
                paradisematic country
              </p>
            </div>
          </div>
          <div className="row">
            <div className="col-md-6 course d-lg-flex ">
              {/* <div className="col-md-6 course d-lg-flex ftco-animate"> */}
              <div
                className="img"
                style={{ backgroundImage: "url(/assets/images/course-1.jpg)" }}
              ></div>
              <div className="text bg-light p-4">
                <h3>
                  <a href="#">Arts Lesson</a>
                </h3>
                <p className="subheading">
                  <span>Class time:</span> 9:00am - 10am
                </p>
                <p>
                  Separated they live in. A small river named Duden flows by
                  their place and supplies it with the necessary regelialia. It
                  is a paradisematic country
                </p>
              </div>
            </div>
            <div className="col-md-6 course d-lg-flex ">
              {/* <div className="col-md-6 course d-lg-flex ftco-animate"> */}
              <div
                className="img"
                style={{ backgroundImage: "url(/assets/images/course-2.jpg)" }}
              ></div>
              <div className="text bg-light p-4">
                <h3>
                  <a href="#">Language Lesson</a>
                </h3>
                <p className="subheading">
                  <span>Class time:</span> 9:00am - 10am
                </p>
                <p>
                  Separated they live in. A small river named Duden flows by
                  their place and supplies it with the necessary regelialia. It
                  is a paradisematic country
                </p>
              </div>
            </div>
            <div className="col-md-6 course d-lg-flex ">
              {/* <div className="col-md-6 course d-lg-flex ftco-animate"> */}
              <div
                className="img"
                style={{ backgroundImage: "url(/assets/images/course-3.jpg)" }}
              ></div>
              <div className="text bg-light p-4">
                <h3>
                  <a href="#">Music Lesson</a>
                </h3>
                <p className="subheading">
                  <span>Class time:</span> 9:00am - 10am
                </p>
                <p>
                  Separated they live in. A small river named Duden flows by
                  their place and supplies it with the necessary regelialia. It
                  is a paradisematic country
                </p>
              </div>
            </div>
            <div className="col-md-6 course d-lg-flex ">
              {/* <div className="col-md-6 course d-lg-flex ftco-animate"> */}
              <div
                className="img"
                style={{ backgroundImage: " url(/assets/images/course-4.jpg)" }}
              ></div>
              <div className="text bg-light p-4">
                <h3>
                  <a href="#">Sports Lesson</a>
                </h3>
                <p className="subheading">
                  <span>Class time:</span> 9:00am - 10am
                </p>
                <p>
                  Separated they live in. A small river named Duden flows by
                  their place and supplies it with the necessary regelialia. It
                  is a paradisematic country
                </p>
              </div>
            </div>
          </div>
        </div>
      </section>
    </>
  );
};

export default CourseSection;
